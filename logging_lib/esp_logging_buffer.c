#include "esp_logging_buffer.h"
 ...
 int32_t buffer_write(uint8_t data_buff[BUFFER_SIZE], uint8_t ui8_data) {
  ...
  int32_t buffer_write_log(uint8_t data_buff[BUFFER_SIZE], uint8_t temp_buff[MAX_TEMP_BUFF], uint8_t log_arg_num) {
 ...
 int32_t buffer_read(uint8_t data_buff[BUFFER_SIZE], uint8_t* pui8_data) {
