...
#include "esp_logging_buffer.h"
 ...
 static uint16_t current_log_level = 8;       //debug level
 static uint32_t current_log_capacity = 1024; //in bytes

 uint8_t data_buff[BUFFER_SIZE];

 static void log_vmi(uint8_t temp_buff[MAX_TEMP_BUFF], uint16_t ui16_vmi_packed, uint8_t * ui8_log_size);

 static uint16_t packed_vmi_log(log_level_t log_level, uint16_t ui16_module, uint16_t ui16_id);

 static void log_arg(uint8_t temp_buff[MAX_TEMP_BUFF], const uint8_t * const pui8_arg, uint8_t * ui8_log_size);

 static void log_msg_end(uint8_t temp_buff[MAX_TEMP_BUFF], uint8_t * ui8_log_size );
 static void log_msg_next_arg(uint8_t temp_buff[MAX_TEMP_BUFF], uint8_t * ui8_log_size );
 ...
 void log_capacity_set(uint16_t ui16_log_capacity) {
 ...
 static void log_vmi( uint8_t temp_buff[MAX_TEMP_BUFF], uint16_t ui16_vmi_packed, uint8_t * ui8_log_size ) {
    ...
    temp_buff[0] = ((uint8_t)(ui16_vmi_packed >> 8));
    temp_buff[1] = ((uint8_t)(ui16_vmi_packed & 0xFF));
    (*ui8_log_size) = 2;
 ...
 static inline void log_msg_end( uint8_t temp_buff[MAX_TEMP_BUFF], uint8_t * ui8_log_size ) {

    temp_buff[(*ui8_log_size)] = (uint8_t)MSG_END;
    (*ui8_log_size)++;

 }

 static inline void log_msg_next_arg( uint8_t temp_buff[MAX_TEMP_BUFF], uint8_t * ui8_log_size ) {

    temp_buff[(*ui8_log_size)] = (uint8_t)NEXT_ARG;
    (*ui8_log_size)++;
 ...
 void log_msg_0( log_level_t log_level, uint16_t ui16_module, uint16_t ui16_id ) {
     ...
     uint8_t temp_buff[MAX_TEMP_BUFF];
     uint8_t ui8_log_size = 0;
    ...
    if (current_log_level >= log_level) {
        ...
        if ( buffer_write_log(data_buff, temp_buff, ui8_log_size) != 0) {
            //BAFFER JE PUN, TREBA SLATI PODaTKE
            //dodati fi-ju
            void log_init();
        }
